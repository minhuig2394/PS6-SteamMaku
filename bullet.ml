...
let update_ufos (bul:bullet) (updates:total_update)= 
  ...
  print_endline "updating ufos";
    ...
    else (fst acc),((set_pos_ufo updated_ufo)::(snd acc)) in
...
let determine (hitufo: bool)(h:bullet) (player:player_char) 
  ...
  if (out h) then 
    ...
    if hit h player then 
      if invincible = true then (add_update (DeleteBullet h.b_id);update t pwr)
      else 
...
let update_bullets (rp:player_char) (bp:player_char) (rinvincible :bool) 
  ...
  let rec update bulletlst pwr= 
    ...
    in updates.ulst <- u; begin 
      ...
      |Blue -> determine hitufo h rp rinvincible update updates t pwr
      end
    |[] -> (updates,pwr)
  in update blst pwr
